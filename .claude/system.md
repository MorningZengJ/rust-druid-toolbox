...
src/
  ...
  model/               → 数据模型
    ...
    rename_state.rs    → RenameState（重命名页状态）、FilterItem（过滤条件）
    ...
    rule_template.rs   → RuleTemplate 枚举（AddPrefixNumber/AddSuffixNumber/SpaceToUnderscore/ToLowercase/RemoveDigitsBeforeExt/Custom）
    ascii_art_state.rs → AsciiArtState（字符画状态）、CharsetPreset、ColorMode、Background 枚举
  themes/              → 主题系统
    mod.rs             → Theme 枚举 (Light/Dark)，get_theme() 工厂函数
    background.rs      → 背景色定义（nav_bg/main_bg/card_bg/header_bg/border_color/table_header_bg/selected_row_bg/status_success_bg/status_error_bg/accent_color/success_color/warning_color/error_color）
    border.rs          → 边框样式定义
    text.rs            → 文本颜色定义（main_text_color/secondary_text_color/muted_text_color）
  ui/                  → UI 层
    ...
    rename/            → 重命名页面（拆分为子模块）
      mod.rs           → Rename 组件（PageWithNav trait），Message 枚举
      file_list.rs     → 文件列表视图（含预览列、选择、双击交互）
      logic.rs         → apply_replace_rules() 替换逻辑、validate_regex() 验证，含单元测试
      replace_rules.rs → 替换规则列表视图（含正则/启用切换）
      status_bar.rs    → 重命名结果状态栏（成功/失败提示）
    ascii_art/         → 字符画页面
...
3. 用户添加多条替换规则（每条含 content/target/enable/is_regex），支持模板快速添加
4. 预览：`logic::apply_replace_rules()` 对文件名逐条应用启用的规则，文件列表实时显示预览
5. 确认弹窗 → 执行：遍历 `filter_file_list`，对每个文件应用所有启用规则后调用 `std::fs::rename()`，返回 RenameResult
...
- `assets/svg/`：UI 图标资源（folder_open/border_color/image/settings/delete_outline 等）
