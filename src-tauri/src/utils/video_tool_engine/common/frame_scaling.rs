...
pub(crate) fn calc_aspect_ratio_resize(
    ...
    let (sw, sh) = if src_ratio > target_ratio {
        (target_width, ((target_width as f64 / src_ratio) as u32).max(1))
    } else {
        (((target_height as f64 * src_ratio) as u32).max(1), target_height)
    };
    (sw / 2 * 2, sh / 2 * 2)
}
...
pub(crate) fn query_audio_encoder_params(
    ...
    let format = if let Some(ref codec) = audio_codec {
        if let Some(formats) = codec.formats() {
            ...
            if supported.contains(&dec_format) { dec_format }
            else { supported.into_iter().next().unwrap_or(dec_format) }
        } else { dec_format }
    } else { dec_format };
    ...
    let rate = if let Some(ref codec) = audio_codec {
        if let Some(rates) = codec.rates() {
            ...
            if supported.is_empty() { dec_rate }
            else if supported.contains(&(dec_rate as i32)) { dec_rate }
            else { supported.into_iter().min_by_key(|r| (*r - dec_rate as i32).unsigned_abs()).map(|r| r as u32).unwrap_or(dec_rate) }
        } else { dec_rate }
    } else { dec_rate };
    ...
    let ch_layout = if let Some(ref codec) = audio_codec {
        ...
        } else if dec_channels >= 2 { ffmpeg_next::channel_layout::ChannelLayout::STEREO }
        else { ffmpeg_next::channel_layout::ChannelLayout::MONO }
    } else if dec_channels >= 2 { ffmpeg_next::channel_layout::ChannelLayout::STEREO }
    else { ffmpeg_next::channel_layout::ChannelLayout::MONO };
...
pub(crate) fn scale_and_pad_frame(
    ...
    if needs_padding {
        let mut scaled_frame = ffmpeg_next::util::frame::video::Video::new(
            ffmpeg_next::format::Pixel::YUV420P, scaled_width, scaled_height,
        );
        sws_ctx.run(decoded, &mut scaled_frame).map_err(|e| anyhow!("颜色空间转换失败: {}", e))?;
        ...
        for row in 0..height as usize { yuv_frame.data_mut(0)[row*dst_y..row*dst_y + width as usize].fill(0); }
        for row in 0..(height/2) as usize { yuv_frame.data_mut(1)[row*dst_u..row*dst_u + (width/2) as usize].fill(128); }
        for row in 0..(height/2) as usize { yuv_frame.data_mut(2)[row*dst_v..row*dst_v + (width/2) as usize].fill(128); }

        for row in 0..scaled_height as usize {
            let s = row * src_y; let d = (row + y_offset as usize) * dst_y + x_offset as usize;
            yuv_frame.data_mut(0)[d..d + scaled_width as usize].copy_from_slice(&scaled_frame.data(0)[s..s + scaled_width as usize]);
        }
        for row in 0..(scaled_height/2) as usize {
            let s = row * src_u; let d = (row + (y_offset/2) as usize) * dst_u + (x_offset/2) as usize;
            yuv_frame.data_mut(1)[d..d + (scaled_width/2) as usize].copy_from_slice(&scaled_frame.data(1)[s..s + (scaled_width/2) as usize]);
        }
        for row in 0..(scaled_height/2) as usize {
            let s = row * src_v; let d = (row + (y_offset/2) as usize) * dst_v + (x_offset/2) as usize;
            yuv_frame.data_mut(2)[d..d + (scaled_width/2) as usize].copy_from_slice(&scaled_frame.data(2)[s..s + (scaled_width/2) as usize]);
        }
    } else {
        sws_ctx.run(decoded, &mut yuv_frame).map_err(|e| anyhow!("颜色空间转换失败: {}", e))?;
    }
