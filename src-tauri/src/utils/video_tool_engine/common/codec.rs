...
pub(crate) fn find_video_encoder_for_format(
    ...
    candidates
        ...
        .map(String::from)
        .ok_or_else(move || {
            anyhow!("未找到可用的视频编码器 ({}). 请确保已安装包含 libx264 的 FFmpeg.", format)
        })
}
...
pub(crate) fn find_audio_encoder_for_codec(codec_name: &str) -> Result<&'static str> {
    let candidates: &[&str] = match codec_name {
        ...
        _ => {
            return if ffmpeg_next::codec::encoder::find_by_name(codec_name).is_some() {
                let all = &["libmp3lame","mp3","aac","libfdk_aac","pcm_s16le","flac","libvorbis","vorbis","libopus","opus","alac","ac3"];
                all.iter().find(|&&n| n == codec_name).copied()
    ...
    candidates.iter().find(|&&name| ffmpeg_next::codec::encoder::find_by_name(name).is_some()).copied()
        .ok_or_else(|| {
            let available = get_available_audio_formats();
            if available.is_empty() {
...
fn get_available_audio_formats() -> Vec<&'static str> {
    let format_encoders: &[(&str, &[&str])] = &[
        ("mp3", &["libmp3lame", "mp3"]), ("aac", &["aac", "libfdk_aac"]),
        ("wav", &["pcm_s16le"]), ("flac", &["flac"]),
        ("ogg", &["libvorbis", "vorbis"]), ("opus", &["libopus", "opus"]),
        ("alac", &["alac"]), ("ac3", &["ac3"]),
    ];
    format_encoders.iter()
        .filter(|(_, encoders)| encoders.iter().any(|&name| ffmpeg_next::codec::encoder::find_by_name(name).is_some()))
        .map(|(format, _)| *format)
        .collect()
}
